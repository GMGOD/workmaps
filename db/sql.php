...
case 23: #SUBIR IMAGEN USUARIO
	if(isset($_POST))
	{
		...
		$BigImageMaxSize 		= 700; //Image Maximum height or width
		$ThumbPrefix			= "thumb_"; //Normal thumb Prefix
		$DestinationDirectory	= realpath('./uploads/usuario/'); //specify upload directory ends with / (slash)
		$Quality 				= 90; //jpeg quality
		##########################################
		...
		if(resizeImage($CurWidth,$CurHeight,$BigImageMaxSize,$DestRandImageName,$CreatedImage,$Quality,$ImageType))
		{
			...
			// Insert info into database table!
			$id_txtUsuario		=	trim($_REQUEST['id_txtUsuario']);
			if($objDB->insertarImagen($id_txtUsuario,$NewImageName,$DestRandImageName,$thumb_DestRandImageName,$DestinationDirectory,$ImageType,$ImageExt,$ImageSize)){
				echo 'db/uploads/usuario/'.$ThumbPrefix.$NewImageName;
			#No cierro la conexion por que es un insertado asincrono...
			}else{
				die('ERROR insertando imagen a DB');
			}
