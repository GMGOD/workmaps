...
class funciones {
	...
	function datetime_diff($older, $newer=NULL, $format='s') {
		...
		if(!in_array($format, array('s', 'm', 'h', 'd'))) {
			$format = 's';
		}
	...
	##Funcion mala (funciones anonimas) $cmpLangs = function($a, $b) use ($lang2pref) {
	...
	*/}
	 ...
	 */
	 ##Funcion mala ( funciones anonimas ) $find = function ( $search, &$key ) use ( $result ) {
...
*/
	...
	*/}
	...
	function settings_has_db_info() {
		...
		$has = true;
		foreach(array('db_driver', 'db_host', 'db_name', 'db_user') as $v) {
			if(!isset($settings[$v])) {
				$has = false;
				break;
			}
